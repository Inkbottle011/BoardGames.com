...
class CleanInactiveLobbies extends Command
{
    ...
    protected $description = 'Remove players who have been inactive for over 1 minute in waiting games';

    public function handle()
    {
        // Find players in waiting games who haven't sent a heartbeat in 1 minute
        $inactive = GamePlayer::whereHas('game', fn($q) => $q->where('status', 'waiting'))
                              ->where('last_seen', '<', now()->subMinute())
